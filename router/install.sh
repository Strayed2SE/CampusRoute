...
FILES="etc/config/campus_route etc/init.d/campus-route etc/hotplug.d/iface/95-campus-route etc/hotplug.d/net/95-campus-usb etc/cron.d/campus-route snapshot.sh rollback.sh usr/bin/campus-route usr/bin/campus-route-update usr/bin/campus-route-rollback usr/lib/lua/luci/controller/campus_route.lua usr/lib/lua/luci/model/cbi/campus_route/main.lua usr/share/rpcd/acl.d/luci-app-campus-route.json"
for rel in $FILES; do
...
if [ "$DRY" -eq 0 ]; then
	...
	chmod 0755 "$ROOT/etc/init.d/campus-route" "$ROOT/etc/hotplug.d/iface/95-campus-route" "$ROOT/etc/hotplug.d/net/95-campus-usb" "$ROOT/snapshot.sh" "$ROOT/rollback.sh" "$ROOT/usr/bin/campus-route" "$ROOT/usr/bin/campus-route-update" "$ROOT/usr/bin/campus-route-rollback"
